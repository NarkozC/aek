...
class General_Model extends CI_Model
{
	...
	*/
	...
	public function UpdateDersler($CData){
		foreach ($CData as $key => $value) {
	       	if (is_null($value)) {
	            $CData[$key] = "";
	        }
	...
	*/
	public function GetIfy()
	{
		...
		foreach ($query as $item) {
		  ...
		  $this->db->where('No', $item->No);
		  $this->db->update('general_ify', $field);
		  $i++;
		}
	...
	public function AddIfy($CData){
		...
		$this->db->insert('general_ify', $Data);
		if($this->db->affected_rows() > 0){
			return true;
		} else{
			return false;
	...
	public function EditIfy($listOrder = -1){
		if ($listOrder != -1) {
			$this->db->where('ListOrder', $listOrder);
	...
	public function UpdateIfy($CData){
		$No = $this->input->post('No');
		foreach ($CData as $key => $value) {
	       	if (is_null($value)) {
	            $CData[$key] = "";
  ...
  public function UpdateResimler($data){
	...
	*/
	...
	public function AddKategoriler($CData){
		foreach ($CData as $key => $value) {
	       	if (is_null($value)) {
	            $CData[$key] = "";
	        }
	...
	public function UpdateKategoriler($CData){
		foreach ($CData as $key => $value) {
	       	if (is_null($value)) {
	            $CData[$key] = "";
	        }
		...
		foreach ($query as $item) {
			...
			$this->db->update('genel_kategoriler', $field);
			if($this->db->affected_rows() > 0){
				rename('resources/images/'.$item->Isim, 'resources/images/'.$CData['Isim']);
				return true;
			}else{
				return false;
			}
