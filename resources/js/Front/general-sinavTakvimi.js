...
function GetOkullarSelect() {
    ...
    $('#' + tr_ID).on('change', function(e) {
        var valueSelected = this.value;
        var yilsValue = $('#' + vars.sectionSPs.Yil + 'Select').selectpicker('val');
        if (yilsValue != '') {
...
function GetYillarSelect() {
    ...
    $('#' + tr_ID).on('change', function(e) {
        var valueSelected = this.value;
        var okulsValue = $('#' + vars.sectionSPs.Okul + 'Select').selectpicker('val');
        if (okulsValue != '') {
            GetSubelerSelect(okulsValue);
            $('#' + vars.sectionSPs.Ay + 'Select').prop('disabled', false);
            RefreshSelectpicker();
            $('#' + vars.sectionButtons.ShowTable).prop('disabled', false);
        }
...
function CreateSelectedTable() {
    var data = vars.sectionDatas.SinavTakvimi.FData,
        html = '',
        searchText = '',
        trArray = new Array('Tarih', 'Sube', 'Ders', 'Aciklama'),
        selOkul = $('#' + vars.sectionSPs.Okul + 'Select').selectpicker('val'),
        selYil = $('#' + vars.sectionSPs.Yil + 'Select').selectpicker('val'),
    ...
    if (selOkul != '') {
        curData = data.filter(function(sinav) {
            return sinav.Okul == selOkul;
        });
    }
    if (selYil != '') {
        curData = data.filter(function(sinav) {
            return sinav.Yil == selYil;
        });
    }
...
function GetSinavTakvimiHtml() {
    ...
    html += '<section id="' + vars.sectionNames.Lower + '">' +
        ...
        '<div class="row">' +
        '<div class="col-xs-0 col-sm-0 col-md-2 col-lg-2 hidden-xs hidden-sm">' +
        '</div>' +
