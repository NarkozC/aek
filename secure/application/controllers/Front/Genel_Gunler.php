...
class Genel_Gunler extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data = array('cached' => false, 'data' => '', 'cachedataTR' => '', 'cachedataEN' => '', 'cacheTime' => 172800,
						'cacheKeys' => array(
                          'GetGunlerTR' => md5('GetGunlerCacheTR'),
                          'GetGunlerEN' => md5('GetGunlerCacheEN'),
                        ),
	...
	public function GetGunler(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetGunlerTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetGunlerEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetGunler();
			} else {
