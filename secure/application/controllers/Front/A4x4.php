...
class A4x4 extends CI_Controller {
	...
	{
		$this->load->view('Front/A4x4-view');
	}
	...
	public function GetA4x4(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetA4x4TR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetA4x4EN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetA4x4();
			} else {
	...
	public function GetA4x4Num(){
		if(! $this->input->is_ajax_request()) {
			...
			if($this->session->userdata('logged_in')) {
				$result = $this->General_Model->GetA4x4Num();
