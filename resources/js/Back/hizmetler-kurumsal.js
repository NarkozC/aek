 var Form =  $('#kurHizmet-form');
 var Modal = $('#kurHizmet-modal');
  $(function(){

    RefreshData()

    $('#addkurHizmetButton').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
          GetResimler();
          $(Form).attr('action', baseurl+'Ap/Hizmetler-Kurumsal/AddKurumsalHizmet');
          // reset the Form
          $(Form)[0].reset();
    ...
    $('#KurumsalHizmetlerSubmit').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
        ...
        $.ajax({
          ...
          success: function(response){
            ...
            if(response.success){
              ...
              iziToast.success({
                  title: 'OK',
                  message: 'Kurumsal Hizmet başarıyla '+type,
              });
            } else {
  ...
  $('#showdata').on('click', '.item-edit', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $(Form).attr('action', baseurl+'Ap/Hizmetler-Kurumsal/UpdateKurumsalHizmet');
      GetResimler();
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hizmetler-Kurumsal/EditKurumsalHizmet',
        data: {No: No},
        async: false,
        dataType: 'json',
  ...
  $('#showdata').on('click', '.item-up', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hizmetler-Kurumsal/UpKurumsalHizmet',
        ...
        success: function(result){
          if(result.success){
            ...
            iziToast.success({
                title: 'OK',
                message: 'Kurumsal Hizmet başarıyla güncellendi!',
            });
          } else {
            RefreshData()
  ...
  $('#showdata').on('click', '.item-down', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hizmetler-Kurumsal/DownKurumsalHizmet',
        data: {No: No, ListOrder: ListOrder},
        async: false,
        dataType: 'json',
        success: function(result){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $('#btnDelete').unbind().click(function(){
        $.ajax({
          ...
          method: 'post',
          async: false,
          url: baseurl+'Ap/Hizmetler-Kurumsal/DeleteKurumsalHizmet',
          data:{No:No},
          dataType: 'json',
          success: function(result){
            if(result.success){
              ...
              iziToast.success({
                title: 'OK',
                message: 'Kurumsal Hizmet başarıyla silindi!',
              });
            } else {
              RefreshData()
  ...
  var isGetKurumsalHizmetlerFirst = true;
  //functions
  function GetKurumsalHizmetler(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#showdata').html(html);
        if (!isGetKurumsalHizmetlerFirst) {
          CreateDataTables();
        }
        isGetKurumsalHizmetlerFirst = false;
      },
      error: function(){
        iziToast.error({
            title: 'Hata!',
  ...
  function GetKurumsalHizmetlerNum(){
    var url = baseurl+'Kurumsal-Hizmetler/GetKurumsalHizmetlerNum';
  ...
  function RefreshData(){
    GetKurumsalHizmetler();
    GetKurumsalHizmetlerNum();
    $(function(){
      RefreshSideData();
    });
  }
