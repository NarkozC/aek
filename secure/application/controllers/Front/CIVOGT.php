...
class CIVOGT extends CI_Controller {
	...
	public function GetCivogt(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetCivogtTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetCivogtEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetCivogt();
			} else {
	...
	public function GetCivogtNum(){
		if(! $this->input->is_ajax_request()) {
			...
			if($this->session->userdata('logged_in')) {
				$result = $this->General_Model->GetCivogtNum();
