 var Form =  $('#referanslar-form');
 var Modal = $('#referanslar-modal');
  $(function(){

    RefreshData()

    $('#addreferanslarButton').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
          GetResimler();
          $(Form).attr('action', baseurl+'Ap/Hakkimizda-Referanslar/AddReferans');
          // reset the Form
          $(Form)[0].reset();
    ...
    $('#ReferanslarSubmit').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
        ...
        $.ajax({
          ...
          success: function(response){
            ...
            if(response.success){
              ...
              iziToast.success({
                  title: 'OK',
                  message: 'Referans başarıyla '+type,
              });
            } else {
  ...
  $('#showdata').on('click', '.item-edit', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $(Form).attr('action', baseurl+'Ap/Hakkimizda-Referanslar/UpdateReferans');
      GetResimler();
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hakkimizda-Referanslar/EditReferans',
        data: {No: No},
        async: false,
        dataType: 'json',
  ...
  $('#showdata').on('click', '.item-up', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hakkimizda-Referanslar/UpReferans',
  ...
  $('#showdata').on('click', '.item-down', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hakkimizda-Referanslar/DownReferans',
        data: {No: No, ListOrder: ListOrder},
        async: false,
        dataType: 'json',
        success: function(result){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $('#btnDelete').unbind().click(function(){
        $.ajax({
          ...
          method: 'post',
          async: false,
          url: baseurl+'Ap/Hakkimizda-Referanslar/DeleteReferans',
          data:{No:No},
          dataType: 'json',
          success: function(result){
            if(result.success){
  ...
  var isGetReferanslarFirst = true;
  //functions
  function GetReferanslar(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#showdata').html(html);
        if (!isGetReferanslarFirst) {
          CreateDataTables();
        }
        isGetReferanslarFirst = false;
      },
      error: function(){
        iziToast.error({
            title: 'Hata!',
  ...
  function GetReferanslarNum(){
    var url = baseurl+'Hakkimizda/GetReferanslarNum';
  ...
  function GetHtmlTr(data){
    ...
    newHtml = '<td>'+
                '<a class="LGM" href="'+baseurl+'resources/images/'+Resim+'" data-sub-html="Isim: '+Isim+'">'+
