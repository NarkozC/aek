    ...
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        if ($en == "true") {
          ...
          $this->form_validation->set_rules('Sinif', FORM_LANG_EN['Sinif'], 'required|trim|max_length[255]|strip_tags|xss_clean'); 
        ...
        if ($this->form_validation->run()) {
          $result = $this->General_Model->AddSinavTarihleri();
          if ($result) {
  ...
  public function UpdateSinavTarihleri(){
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        $this->form_validation->set_error_delimiters('<p class="text-danger">', '</p>');
        if ($this->form_validation->run()) {
          $result = $this->General_Model->UpdateSinavTarihleri();
