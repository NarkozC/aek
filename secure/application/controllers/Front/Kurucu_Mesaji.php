...
class Kurucu_Mesaji extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data = array('cached' => false, 'data' => '', 'cachedataTR' => '', 'cachedataEN' => '', 'cacheTime' => 172800,
						'cacheKeys' => array(
                          ...
                          'GetKurucuMesajiHtmlTR' => md5('GetKurucuMesajiHtmlCacheTR'),
                          'GetKurucuMesajiHtmlEN' => md5('GetKurucuMesajiHtmlCacheEN'),
                        ),
	...
	{
		$this->load->view('Front/KurucuMesaji-view');
	}
	...
	public function GetKurucuMesaji(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetKurucuMesajiTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetKurucuMesajiEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetKurucuMesaji();
			} else {
