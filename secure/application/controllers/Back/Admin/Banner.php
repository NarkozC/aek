...
class Banner extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->data = array('success' => false, 'messages' => array(), 'type' => 'add',
            'cacheKeys' => array(
                ...
                'GetBannerTR' => md5('GetBannerCacheTR'),
                'GetBannerEN' => md5('GetBannerCacheEN'),
            ),
        );
    ...
    public function index()
    {
        if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
            $this->load->view('Back/Admin/Banner-view');
        } else {
            redirect('Portal');
        }
    }
        ...
        if(! $this->input->is_ajax_request()) {
            ...
            if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                ...
                if ($this->form_validation->run()) {
                    $result = $this->General_Model->AddBanner();
                    if ($result) {
                        $cacheKeys = array('Banner');
                        ...
                        $this->data['data'] = $this->General_Model->EditBanner('0');
        ...
        public function UpdateBanner(){
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        $result = $this->General_Model->UpdateBanner();
                        ...
                        if($result){
                            ...
                            $this->data['data'] = $this->General_Model->EditBanner();
                            $this->data['success'] = true;
                        }
                    } else {
                        foreach ($_POST as $key => $value) {
        ...
        public function DeleteBanner(){
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        $result = $this->General_Model->DeleteBanner();
                        if($result){
                            $cacheKeys = array('Banner');
                            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
                                $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
            ...
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        $result = $this->General_Model->UpBanner();
                        if($result){
                            $cacheKeys = array('Banner');
                            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
                                $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
        ...
        public function DownBanner(){
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        $result = $this->General_Model->DownBanner();
                        if($result){
                            $cacheKeys = array('Banner');
                            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
                                $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
