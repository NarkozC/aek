 var Form =  $('#bannerSlider-form');
 var Modal = $('#bannerSlider-modal');
  $(function(){

    RefreshData()

    $('#addbannerSliderButton').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
          GetResimler();
          $(Form).attr('action', baseurl+'Ap/Anasayfa-Banner/AddBannerSlide');
          // reset the Form
          $(Form)[0].reset();
    ...
    $('#BannerSliderSubmit').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
        if ($('#en_Resim').selectpicker('val') == "0") {
          $('#en_Resim').selectpicker('val', '');
        }
        if ($('#en_Arkaplan').selectpicker('val') == "0") {
          $('#en_Arkaplan').selectpicker('val', '');
        }
        ...
        $.ajax({
          ...
          success: function(response){
            ...
            if(response.success){
              ...
              iziToast.success({
                  title: 'OK',
                  message: 'Banner Slide başarıyla '+type,
              });
            } else {
  ...
  $('#showdata').on('click', '.item-edit', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $(Form).attr('action', baseurl+'Ap/Anasayfa-Banner/UpdateBannerSlide');
      GetResimler();
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Anasayfa-Banner/EditBannerSlide',
        data: {No: No},
        async: false,
        dataType: 'json',
        success: function(result){
          ...
          if(result.success){
            ...
            $('#tr_Arkaplan').selectpicker('val', result.data.tr_Arkaplan);
            $('#en_Arkaplan').selectpicker('val', result.data.en_Arkaplan);
  ...
  $('#showdata').on('click', '.item-up', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Anasayfa-Banner/UpBannerSlide',
        ...
        success: function(result){
          if(result.success){
            ...
            iziToast.success({
                title: 'OK',
                message: 'Banner Slide başarıyla güncellendi!',
            });
          } else {
            RefreshData()
  ...
  $('#showdata').on('click', '.item-down', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Anasayfa-Banner/DownBannerSlide',
        data: {No: No, ListOrder: ListOrder},
        async: false,
        dataType: 'json',
        success: function(result){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $('#btnDelete').unbind().click(function(){
        $.ajax({
          ...
          method: 'post',
          async: false,
          url: baseurl+'Ap/Anasayfa-Banner/DeleteBannerSlide',
          data:{No:No},
          dataType: 'json',
          success: function(result){
            if(result.success){
              ...
              iziToast.success({
                title: 'OK',
                message: 'Banner Slide başarıyla silindi!',
              });
            } else {
              RefreshData()
  ...
  var isGetBannerSliderFirst = true;
  //functions
  function GetBannerSlider(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#showdata').html(html);
        if (!isGetBannerSliderFirst) {
          CreateDataTables();
        }
        isGetBannerSliderFirst = false;
      },
      error: function(){
        iziToast.error({
            title: 'Hata!',
  ...
  function GetResimler(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        var Atr_html = '<select class="form-control selectpicker" data-live-search="true" name="tr_Arkaplan" id="tr_Arkaplan" title="Arkaplan Seç..." data-liveSearchNormalize="true">';
        ...
        var Aen_html = '<select class="form-control selectpicker" data-live-search="true" name="en_Arkaplan" id="en_Arkaplan" title="Arkaplan Seç..." data-liveSearchNormalize="true">'+'<option data-tokens="Boş Sil" value="0">Türkçe Resmi Kullan</option>';
        var i;
        ...
        for(i=0; i<data.length; i++){
          ...
          Atr_html += '<option data-tokens="'+data[i].Kategori+'/'+data[i].Dosya+' '+data[i].Isim+' '+data[i].Kategori+'" value="'+data[i].Kategori+'/'+data[i].Dosya+'">'+data[i].Isim+' ('+data[i].Kategori+')</option>';
          Aen_html += '<option data-tokens="'+data[i].Kategori+'/'+data[i].Dosya+' '+data[i].Isim+' '+data[i].Kategori+'" value="'+data[i].Kategori+'/'+data[i].Dosya+'">'+data[i].Isim+' ('+data[i].Kategori+')</option>';
        }
        tr_html += '</select>'
        en_html += '</select>'
        Atr_html += '</select>'
        ...
        $('#tr_Arkaplanlar').html(Atr_html);
        $('#en_Arkaplanlar').html(Aen_html);
  ...
  function GetBannerSliderNum(){
    var url = baseurl+'Anasayfa/GetBannerSliderNum';
  ...
  function GetHtmlTr(data){
    ...
    var TRArkaplan = data.tr_Arkaplan;

    var ENResim = data.en_Resim;
    var ENArkaplan = data.en_Arkaplan;
    ...
    if (ENArkaplan == "") {
      ...
      Arkaplan = TRArkaplan+' | '+ENArkaplan;
    }
    ...
    newHtml = '<td>'+
                ...
                '</td>'+
                '<td>'+
                '<a class="LGM" href="'+baseurl+'resources/images/'+Arkaplan+'" data-sub-html="Arkaplan: '+Arkaplan+'">'+
                  '<span style="color:black;" class="shorten_content6">'+Arkaplan+'</span>'+
                '</a>'+
                '</td>'+
                '<td>'+
