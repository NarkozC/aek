 var Form =  $('#birEgitim-form');
 var Modal = $('#birEgitim-modal');
  $(function(){

    RefreshData()

    $('#addbirEgitimButton').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
          GetResimler();
          $(Form).attr('action', baseurl+'Ap/Egitimler-Bireysel/AddBireyselEgitim');
          // reset the Form
          $(Form)[0].reset();
    ...
    $('#BireyselEgitimlerSubmit').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
        ...
        $.ajax({
          ...
          success: function(response){
            ...
            if(response.success){
              ...
              iziToast.success({
                  title: 'OK',
                  message: 'Bireysel Egitim başarıyla '+type,
              });
            } else {
  ...
  $('#showdata').on('click', '.item-edit', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $(Form).attr('action', baseurl+'Ap/Egitimler-Bireysel/UpdateBireyselEgitim');
      GetResimler();
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Egitimler-Bireysel/EditBireyselEgitim',
        data: {No: No},
        async: false,
        dataType: 'json',
  ...
  $('#showdata').on('click', '.item-up', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Egitimler-Bireysel/UpBireyselEgitim',
        ...
        success: function(result){
          if(result.success){
            ...
            iziToast.success({
                title: 'OK',
                message: 'Bireysel Egitim başarıyla güncellendi!',
            });
          } else {
            RefreshData()
  ...
  $('#showdata').on('click', '.item-down', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Egitimler-Bireysel/DownBireyselEgitim',
        data: {No: No, ListOrder: ListOrder},
        async: false,
        dataType: 'json',
        success: function(result){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $('#btnDelete').unbind().click(function(){
        $.ajax({
          ...
          method: 'post',
          async: false,
          url: baseurl+'Ap/Egitimler-Bireysel/DeleteBireyselEgitim',
          data:{No:No},
          dataType: 'json',
          success: function(result){
            if(result.success){
              ...
              iziToast.success({
                title: 'OK',
                message: 'Bireysel Egitim başarıyla silindi!',
              });
            } else {
              RefreshData()
  ...
  var isGetBireyselEgitimlerFirst = true;
  //functions
  function GetBireyselEgitimler(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#showdata').html(html);
        if (!isGetBireyselEgitimlerFirst) {
          CreateDataTables();
        }
        isGetBireyselEgitimlerFirst = false;
      },
      error: function(){
        iziToast.error({
            title: 'Hata!',
  ...
  function GetBireyselEgitimlerNum(){
    var url = baseurl+'Bireysel-Egitimler/GetBireyselEgitimlerNum';
