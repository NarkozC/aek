...
class Genel_Dersler extends CI_Controller {
	...
	public function GetDersler(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetDerslerTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetDerslerEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetDersler();
			} else {
	...
	public function GetDerslerNum(){
		if(! $this->input->is_ajax_request()) {
			...
			if($this->session->userdata('logged_in')) {
				$result = $this->General_Model->GetDerslerNum();
