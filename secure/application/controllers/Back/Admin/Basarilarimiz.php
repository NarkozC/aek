...
class Basarilarimiz extends CI_Controller {
    ...
    public function index()
    {
        if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
            $this->load->view('Back/Admin/Basarilarimiz-view');
        } else {
            redirect('Portal');
        }
    }

    public function AddBasarilarimiz(){
        if(! $this->input->is_ajax_request()) {
            ...
            if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    $result = $this->General_Model->AddBasarilarimiz($ModelData);
                    if ($result) {
                        $cacheKeys = array('Basarilarimiz');
                        ...
                        $this->data['data'] = $this->General_Model->EditBasarilarimiz('0');
        ...
        public function UpdateBasarilarimiz(){
            if(! $this->input->is_ajax_request()) {
                redirect('404');
            } else {
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        $result = $this->General_Model->UpdateBasarilarimiz($ModelData);
                        ...
                        if($result){
                            ...
                            $this->data['data'] = $this->General_Model->EditBasarilarimiz();
                            $this->data['success'] = true;
                        }
                    } else {
                        foreach ($_POST as $key => $value) {
        ...
        public function EditBasarilarimiz(){
            if(! $this->input->is_ajax_request()) {
                redirect('404');
            } else {
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        public function DeleteBasarilarimiz(){
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        $result = $this->General_Model->DeleteBasarilarimiz();
                        if($result){
                            $cacheKeys = array('Basarilarimiz');
                            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
                                $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
            ...
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        ...
                        $result = $this->General_Model->UpBasarilarimiz();
                        if($result){
                            $cacheKeys = array('Basarilarimiz');
                            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
                                $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
        ...
        public function DownBasarilarimiz(){
            if(! $this->input->is_ajax_request()) {
                ...
                if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
                    ...
                    if ($this->form_validation->run()) {
                        $result = $this->General_Model->DownBasarilarimiz();
                        if($result){
                            $cacheKeys = array('Basarilarimiz');
                            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
                                $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
