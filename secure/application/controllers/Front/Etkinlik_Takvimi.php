...
class Etkinlik_Takvimi extends CI_Controller {
	...
	{
		$this->load->view('Front/EtkinlikTakvimi-view');
	}
	...
	public function GetEtkinlikTakvimi(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetEtkinlikTakvimiTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetEtkinlikTakvimiEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetEtkinlikTakvimi();
			} else {
