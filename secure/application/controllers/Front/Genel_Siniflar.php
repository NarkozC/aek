...
class Genel_Siniflar extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data = array('cached' => false, 'data' => '', 'cachedataTR' => '', 'cachedataEN' => '', 'cacheTime' => 172800,
						'cacheKeys' => array(
                          'GetSiniflarTR' => md5('GetSiniflarCacheTR'),
                          'GetSiniflarEN' => md5('GetSiniflarCacheEN'),
                        ),
	...
	public function GetSiniflar(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetSiniflarTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetSiniflarEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetSiniflar();
			} else {
	...
	public function GetSiniflarNum(){
		if(! $this->input->is_ajax_request()) {
			...
			if($this->session->userdata('logged_in')) {
				$result = $this->General_Model->GetSiniflarNum();
