var vars = {
    ...
    sectionControllers: {
        Normal: baseurl + 'Basarilarimiz/',
        Portal: baseurl + 'Portal/Admin/Basarilarimiz/',
    },
    sectionNames: {
        Normal: 'Başarılarımız',
        Upper: 'Basarilarimiz',
        Lower: 'basarilarimiz',
        Kod: 'GB',
    },
    sectionShowBases: {
        Sections: 'showBasarilarimiz',
        Num: 'showNum',
        Modal: 'showSectionsModal',
    },
    sectionFunctions: {
        Get: 'GetBasarilarimiz',
        Add: 'AddBasarilarimiz',
        Update: 'UpdateBasarilarimiz',
        Edit: 'EditBasarilarimiz',
        Delete: 'DeleteBasarilarimiz',
        Up: 'UpBasarilarimiz',
        Down: 'DownBasarilarimiz',
    },
    sectionButtons: {
        OpenModal: 'BasarilarimizOpenModal',
        Submit: 'BasarilarimizSubmit',
    },
    sectionDatas: {
        Basarilarimiz: {
            Data: new Array(),
...
function GetSectionsNum() {
    $('#' + vars.sectionShowBases.Num).html(vars.sectionDatas.Basarilarimiz.Num);
}
...
function CreateSectionsTable() {
    ...
    $('#show' + vars.sectionNames.Upper + 'Data').html(vars.sectionDatas.Basarilarimiz.BHtml);
...
function GetSectionsData() {
    vars.sectionDatas.Basarilarimiz = {
        Data: new Array(),
        FHtml: '',
        BHtml: '',
        Num: 0,
    ...
    $.ajax({
        ...
        success: function(result) {
            if (en && result.cachedataEN != "") {
                var cache = result.cachedataEN.Basarilarimiz;
                vars.sectionDatas.Basarilarimiz = cache;
                vars.sectionDatas.Basarilarimiz.Data = JSON.parse(cache.Data);
            } else if (!en && result.cachedataTR != "") {
                var cache = result.cachedataTR.Basarilarimiz;
                vars.sectionDatas.Basarilarimiz = cache;
                vars.sectionDatas.Basarilarimiz.Data = JSON.parse(cache.Data);
            } else {
                ...
                for (i = 0; i < length; i++) {
                    ...
                    vars.sectionDatas.Basarilarimiz.Data[i] = curData;
                ...
                vars.sectionDatas.Basarilarimiz.BHtml = bHtml;
                vars.sectionDatas.Basarilarimiz.FHtml = fHtml;
                vars.sectionDatas.Basarilarimiz.Num = length;


                $('#' + vars.sectionShowBases.Sections).html(fHtml);

                var myJSON = JSON.stringify(vars.sectionDatas.Basarilarimiz.Data);
                vars.sectionDatas.Basarilarimiz.Data = myJSON;
                var theCacheData = {
                    Basarilarimiz: vars.sectionDatas.Basarilarimiz,
                }
                setTimeout(Cache('GetSectionsData', url, theCacheData), 1);
                vars.sectionDatas.Basarilarimiz.Data = JSON.parse(myJSON);
            }
        },
        error: function() {
            iziError();
