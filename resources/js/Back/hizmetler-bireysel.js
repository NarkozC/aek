 var Form =  $('#birHizmet-form');
 var Modal = $('#birHizmet-modal');
  $(function(){

    RefreshData()

    $('#addbirHizmetButton').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
          GetResimler();
          $(Form).attr('action', baseurl+'Ap/Hizmetler-Bireysel/AddBireyselHizmet');
          // reset the Form
          $(Form)[0].reset();
    ...
    $('#BireyselHizmetlerSubmit').click(function(e){
      var $link = $(e.target);
      if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
        ...
        $.ajax({
          ...
          success: function(response){
            ...
            if(response.success){
              ...
              iziToast.success({
                  title: 'OK',
                  message: 'Bireysel Hizmet başarıyla '+type,
              });
            } else {
  ...
  $('#showdata').on('click', '.item-edit', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $(Form).attr('action', baseurl+'Ap/Hizmetler-Bireysel/UpdateBireyselHizmet');
      GetResimler();
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hizmetler-Bireysel/EditBireyselHizmet',
        data: {No: No},
        async: false,
        dataType: 'json',
  ...
  $('#showdata').on('click', '.item-up', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hizmetler-Bireysel/UpBireyselHizmet',
        ...
        success: function(result){
          if(result.success){
            ...
            iziToast.success({
                title: 'OK',
                message: 'Bireysel Hizmet başarıyla güncellendi!',
            });
          } else {
            RefreshData()
  ...
  $('#showdata').on('click', '.item-down', function(e){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: baseurl+'Ap/Hizmetler-Bireysel/DownBireyselHizmet',
        data: {No: No, ListOrder: ListOrder},
        async: false,
        dataType: 'json',
        success: function(result){
    ...
    if(!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
      ...
      $('#btnDelete').unbind().click(function(){
        $.ajax({
          ...
          method: 'post',
          async: false,
          url: baseurl+'Ap/Hizmetler-Bireysel/DeleteBireyselHizmet',
          data:{No:No},
          dataType: 'json',
          success: function(result){
            if(result.success){
              ...
              iziToast.success({
                title: 'OK',
                message: 'Bireysel Hizmet başarıyla silindi!',
              });
            } else {
              RefreshData()
  ...
  var isGetBireyselHizmetlerFirst = true;
  //functions
  function GetBireyselHizmetler(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#showdata').html(html);
        if (!isGetBireyselHizmetlerFirst) {
          CreateDataTables();
        }
        isGetBireyselHizmetlerFirst = false;
      },
      error: function(){
        iziToast.error({
            title: 'Hata!',
  ...
  function GetBireyselHizmetlerNum(){
    var url = baseurl+'Bireysel-Hizmetler/GetBireyselHizmetlerNum';
  ...
  function RefreshData(){
    GetBireyselHizmetler();
    GetBireyselHizmetlerNum();
    $(function(){
      RefreshSideData();
    });
  }
