...
class Yde extends CI_Controller {
  ...
  public function __construct()
  {
    ...
    $this->data = array('success' => false, 'messages' => array(), 'type' => 'add',
                        'cacheKeys' => array(
                          ...
                          'GetYdeTR' => md5('GetYdeCacheTR'),
                          'GetYdeEN' => md5('GetYdeCacheEN'),
                        ),
    );
  ...
  public function index()
  {
    if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
      $this->load->view('Back/Admin/Yde-view');
    } else {
      redirect('Portal');
    }
  }

  public function AddYde(){
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        if ($this->form_validation->run()) {
          ...
          $result = $this->General_Model->AddYde($ModelData);
          if ($result) {
            $cacheKeys = array('Yde');
            ...
            $this->data['data'] = $this->General_Model->EditYde('0');
    ...
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        if ($this->form_validation->run()) {
          ...
          $result = $this->General_Model->UpdateYde($ModelData);
          ...
          if($result){
            ...
            $this->data['data'] = $this->General_Model->EditYde();
            $this->data['success'] = true;
          }
        } else {
          foreach ($_POST as $key => $value) {
  ...
  public function DeleteYde(){
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        $this->form_validation->set_error_delimiters('<p class="text-danger">', '</p>');
        if ($this->form_validation->run()) {
          $result = $this->General_Model->DeleteYde();
          if($result){
            $cacheKeys = array('Yde');
            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
              $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
    ...
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        if ($this->form_validation->run()) {
          ...
          $result = $this->General_Model->UpYde();
          if($result){
            $cacheKeys = array('Yde');
            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
              $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
  ...
  public function DownYde(){
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        $this->form_validation->set_error_delimiters('<p class="text-danger">', '</p>');
        if ($this->form_validation->run()) {
          $result = $this->General_Model->DownYde();
          if($result){
            $cacheKeys = array('Yde');
            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
              $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
