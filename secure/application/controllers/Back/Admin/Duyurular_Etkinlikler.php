...
class Duyurular_Etkinlikler extends CI_Controller {
  ...
  public function __construct()
  {
    ...
    $this->data = array('success' => false, 'messages' => array(), 'type' => 'add',
                        'cacheKeys' => array(
                          ...
                          'GetDuyurularEtkinliklerTR' => md5('GetDuyurularEtkinliklerCacheTR'),
                          'GetDuyurularEtkinliklerEN' => md5('GetDuyurularEtkinliklerCacheEN'),

                          'GetDuyurularEtkinliklerHtmlTR' => md5('GetDuyurularEtkinliklerHtmlCacheTR'),
                          'GetDuyurularEtkinliklerHtmlEN' => md5('GetDuyurularEtkinliklerHtmlCacheEN'),
  ...
  public function index()
  {
    if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
      $this->load->view('Back/Admin/DuyurularEtkinlikler-view');
    } else {
      redirect('Portal');
    }
  }

  public function AddDuyurularEtkinlikler(){
    if(! $this->input->is_ajax_request()) {
      ...
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        if ($this->form_validation->run()) {
          ...
          $result = $this->General_Model->AddDuyurularEtkinlikler($ModelData);
          if ($result) {
            $cacheKeys = array('DuyurularEtkinlikler','DuyurularEtkinliklerHtml');
            ...
            $this->data['data'] = $this->General_Model->EditDuyurularEtkinlikler('0');
  ...
  public function UpdateDuyurularEtkinlikler(){
    if(! $this->input->is_ajax_request()) {
      redirect('404');
    } else {
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        if ($this->form_validation->run()) {
          ...
          $result = $this->General_Model->UpdateDuyurularEtkinlikler($ModelData);
          ...
          if($result){
            ...
            $this->data['data'] = $this->General_Model->EditDuyurularEtkinlikler();
            $this->data['sayfa'] = $_POST['Okul'];
            $this->data['success'] = true;
          }
        } else {
  ...
  public function DeleteDuyurularEtkinlikler(){
    if(! $this->input->is_ajax_request()) {
      redirect('404');
    } else {
      if($this->session->userdata('logged_in') && $this->session->userdata('Admin')) {
        ...
        $this->form_validation->set_error_delimiters('<p class="text-danger">', '</p>');
        if ($this->form_validation->run()) {
          $result = $this->General_Model->DeleteDuyurularEtkinlikler();
          if($result){
            $cacheKeys = array('DuyurularEtkinlikler','DuyurularEtkinliklerHtml');
            for ($i=0; $i < sizeof($cacheKeys); $i++) { 
              $cacheKeyTR = $this->data['cacheKeys']['Get'.$cacheKeys[$i].'TR'];
