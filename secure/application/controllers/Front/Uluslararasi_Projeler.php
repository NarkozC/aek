...
class Uluslararasi_Projeler extends CI_Controller {
	...
	{
		$this->load->view('Front/UluslararasiProjeler-view');
	}
	...
	public function GetUluslararasiProjeler(){
		if(! $this->input->is_ajax_request()) {
			...
			$cacheKeyTR = $this->data['cacheKeys']['GetUluslararasiProjelerTR'];
            $cacheKeyEN = $this->data['cacheKeys']['GetUluslararasiProjelerEN'];
			$this->data['English'] = $this->input->post('English');
			$this->data['NeedData'] = $this->input->post('NeedData');

			if ($this->data['NeedData'] == "true") {
				$this->data['data'] = $this->General_Model->GetUluslararasiProjeler();
			} else {
	...
	public function GetUluslararasiProjelerNum(){
		if(! $this->input->is_ajax_request()) {
			...
			if($this->session->userdata('logged_in')) {
				$result = $this->General_Model->GetUluslararasiProjelerNum();
